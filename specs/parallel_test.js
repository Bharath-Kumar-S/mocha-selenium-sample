...
const { Builder, By, Key, until, FileDetector, logging } = require('selenium-webdriver');
...
capabilities.forEach(function(caps) {
  ...
  describe("Lambda Test cretification project " + caps.browserName, async function() {
    ...
    it(`File upload`, async () => {
        ...
        // let fd = await new FileDetector().handleFile(driver, __dirname);
        // await driver.setFileDetector(fd);
        ...
        await (await driver.switchTo().alert()).accept();
        // expect(logs).to.contain(/Image Bharath sucessfully updated : jenkins.svg/)
